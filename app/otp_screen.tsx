...
if (response.ok && result.success === true) {
  // 1. Point to 'existing' instead of 'data'
  ...
  // 2. Map the keys correctly so bvn_validation doesn't get 'undefined'
  router.push({
    pathname: '/bvn_validation', 
    params: { 
      bvn: userData.bvn,
      firstName: userData.first_name,     // API 'first_name' -> App 'firstName'
      lastName: userData.last_name,       // API 'last_name' -> App 'lastName'
      dob: userData.date_of_birth,       // API 'date_of_birth' -> App 'dob'
      email: userData.email,
      address: userData.residential_address, // API 'residential_address' -> App 'address'
      phone: params.phone,               // Carry over the phone from the previous screen
      isNonNigerian: 'false'
    }
  });
} else {
